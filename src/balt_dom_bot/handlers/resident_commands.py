...
Команды работают как в групповом чате ЖК, так и в личном диалоге с ботом.

/help, /start  — справка для жильца
/mystatus      — статус последних обращений этого пользователя в данном чате
...
def _format_mystatus(rows: list) -> str:
    if not rows:
        return (
            "У вас пока нет зафиксированных обращений в этом чате.\n"
            "Просто напишите вопрос — я помогу! 💬"
        )
...
def _format_contacts(complex_name: str, contacts_info: str | None) -> str:
    ...
    return (
        ...
        "По всем вопросам пишите прямо в этот чат — я передам специалисту.\n"
        "По срочным ситуациям обратитесь в аварийную диспетчерскую службу."
    )
