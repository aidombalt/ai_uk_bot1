...
# Склейка разбитых слов: 3+ одиночных символов через пробел → слово.
# {2,} = «(пробел+символ)» повторяется ≥2 раза → всего ≥3 символа.
# Порог 3 защищает «а б» (2 символа), но схлопывает «М е ф» → «Меф»,
# что необходимо для поиска нарко-аббревиатур в drug-маркерах.
_SPACED_WORD_RE = re.compile(r'(?<!\S)\S(?: \S){2,}(?!\S)')
...
# «Пиши в лс/личку/тг» — функциональный аналог @упоминания: спамер даёт
# способ связи без хэндла. «В тг» / «в телеграм» — Telegram как DM-канал.
# Только в спаме; жильцы пишут «напишите мне» или «в нашу группу», не «в тг».
_LS_RE = re.compile(
    r'(?:пиш|напиш)\w{0,5}\s+(?:мне\s+)?в\s+'
    r'(?:лс\b|личку\b|личные\b|тг\b|тлг\b|телеграм\b|telegram\b)'
    r'|\bв\s+(?:лс\b|личку\b|тг\b|тлг\b|телеграм\b|telegram\b)',
...
_DRUG_MARKERS: tuple[str, ...] = (
    ...
    "мефедрон", " меф",   # «М е ф» → «Меф» после {2,} схлопывания
    "амфетамин", "метамфетамин", "метадон", "героин",
...
_SOFT_SPAM_WORDS: tuple[str, ...] = (
    ...
    "берём", "берем",
    # Претензии на доход — «Nк рублей в день/сутки» без трёхзначного числа.
    # Дублирует _INCOME_RE для случаев, когда формат числа не стандартный.
    "рублей в день", "рублей в сутки",
    # Коммерческая реклама услуг (в сочетании с телефоном/упоминанием → спам)
    "акция", "скидк", "бесплатн", "пломбировк", "звоните",
)

# Паттерн аномальных доходов.
# Первая альтернатива: «100к/неделю», «150к в день», «15Ок в мecяц» и т.п.
#   (?:\d{3}|\d{2}[O0оО]) — 3 цифры ИЛИ 2 цифры + О/0 как ноль («15Ок» = «150к»).
# Вторая альтернатива: «5к рублей в день» — однозначное число + «рублей» как квалификатор.
#   Слово «рублей» обязательно: без него «5к» слишком общее (5000 чего угодно).
_INCOME_RE = re.compile(
    r"(?:\d{3}|\d{2}[O0оО])\s*[кkКк]\s*(?:/|в\s*)(?:неделю?\w*|мес(?:яц\w*)?|сутк\w*|день|дн\w*)"
    r"|\d+\s*[кkКк]\s+рублей\s+(?:в\s+)?(?:день|сутк\w*|дн\w*|неделю?\w*|мес(?:яц\w*)?)",
    re.IGNORECASE | re.UNICODE,
)
...
def is_spam_candidate(text: str) -> bool:
    ...
    if _INCOME_RE.search(text_lc) or _INCOME_RE.search(norm):
        return True
    # ❄️ (снежинка) — emoji-маркер кокаина/мефедрона в drug-спаме.
    # Одна ❄️ без 🍁 слабее пары, но в сочетании с контактом достаточно подозрительна
    # для LLM-проверки. Например: «❄️ М е ф пишите в тг!»
    if '❄' in text:    # U+2744 = ❄
        return True
