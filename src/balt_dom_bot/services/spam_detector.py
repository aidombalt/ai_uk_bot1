...
# Паттерн аномальных доходов: «100к/неделю», «150к в день» и т.п.
_INCOME_RE = re.compile(
    r"\d{2,3}\s*[кk]\s*(?:/|в\s*)(?:неделю?|недел\w*|мес(?:яц\w*)?|сутк\w*|день|дн\w*)",
...
def is_spam_candidate(text: str) -> bool:
    ...
    И (коммерческие слова-маркеры ИЛИ паттерн аномального дохода).

    Нормализует текст перед проверкой — ловит обфускацию.
    Намеренно консервативен: лучше пропустить единичный кейс, чем тратить
    LLM-токены на каждое сообщение с @упоминанием.
    """
    ...
    return any(w in norm for w in _SOFT_SPAM_WORDS) or bool(_INCOME_RE.search(norm))
