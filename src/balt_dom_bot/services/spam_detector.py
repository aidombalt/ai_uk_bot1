...
# Таблица: латинский гомоглиф / цифра / спецсимвол → кириллица
_HOMOGLYPH_TABLE: dict[int, str] = {
    ...
    ord('0'): 'о',   # д0ставка → доставка (digit zero как буква о)
    # спецсимволы
...
def _normalize_obfuscated(text: str) -> str:
    ...
    1. Латинские гомоглифы / цифры / спецсимволы → кириллица
       (k→к, 0→о, €→е, e→е, o→о, a→а, c→с, и т.д.)
...
_SOFT_SPAM_WORDS: tuple[str, ...] = (
    ...
    # Для recruitment-спама хватает «курьер», «вакансия», «заработ», «оплат».
...
def is_spam_candidate(text: str) -> bool:
    ...
    Условие: @упоминание (канал контакта — обязательный признак спама)
    И хотя бы один из трёх сигналов:
      1. Коммерческие слова-маркеры в нормализованном тексте.
      2. Паттерн аномального дохода (100к/нед, 15Ок в мecяц и т.п.).
      3. Смешение скриптов внутри слов (р@ботниkи, д0ставkу, Тр€буются) —
         гарантированный catch-all для любой обфускации с @упоминанием.

    Нормализует текст перед проверкой — ловит классические гомоглифы.
    """
    # @упоминание обязательно: спамер всегда указывает контакт
    ...
    if _INCOME_RE.search(text.lower()) or _INCOME_RE.search(norm):
        return True
