...
def is_spam_candidate(text: str) -> bool:
    ...
    Условие: контакт (@ или телефон — спамер ВСЕГДА оставляет способ связи)
    И хотя бы один из трёх сигналов:
      1. Коммерческие / recruitment слова-маркеры в нормализованном тексте.
      2. Паттерн аномального дохода (100к/нед, 15Ок в мecяц и т.п.).
      3. Смешение скриптов внутри слов (р@ботниkи, В0ДОСЧЁТЧИК0В) —
         catch-all для любой новой техники обфускации.
