...
# ВАЖНО: YandexGPT использует разнообразные формулировки отказа в зависимости от
# версии модели и контента. Список намеренно широкий — лучше пропустить легитимный
# нестандартный ответ (который всё равно попадёт в parse_error → is_spam=False),
# чем пропустить реальный отказ на наркотический контент (→ is_spam=False, спам проходит).
_REFUSAL_FRAGMENTS: frozenset[str] = frozenset({
    ...
    "давайте обсудим что",
    "не могу выполнить",
    # Дополнительные формулировки YandexGPT (наблюдались в production)
    "не предназначен для",
    "не могу предоставить",
    "не могу рассматривать",
    "не могу анализировать",
    "не в моей компетенц",
    "данная тема",           # «Данная тема выходит...» / «...не в моей компетенции»
    "подобные запросы",      # «Я не отвечаю на подобные запросы»
    "не отвечаю на",
    "отказываюсь",
    "противоречит",          # «...противоречит моим принципам»
    "нарушает",              # «...нарушает правила использования»
    "запрещённ",             # «...запрещённый контент»
    "незаконн",              # «...незаконные вещества»
    "не имею права",
    "не уполномочен",
...
def _parse_verdict(raw: str) -> SpamLLMVerdict:
    ...
    Логика обработки не-JSON ответов:
      1. Известные фразы отказа (_REFUSAL_FRAGMENTS) → is_spam=True.
         YandexGPT практически никогда не отказывает на легитимные ЖК-сообщения,
         поэтому любой отказ = контент подозрителен.
      2. Непустой не-JSON ответ (< 300 символов) без признаков технического сбоя →
         вероятный неизвестный шаблон отказа → is_spam=True (консервативная политика).
         Граница 300 символов: развёрнутые объяснения (300+) скорее баг модели,
         чем отказ. Пустой ответ или ошибка API → технический сбой → is_spam=False.
      3. Технический сбой (пустая строка, JSON-синтаксис-ошибка) → is_spam=False
         (fail-safe: лучше пропустить единичный спам, чем забанить жильца).
    """
    raw = raw.strip()
    ...
    if not raw:
        log.warning("spam_llm.empty_response")
        return SpamLLMVerdict(is_spam=False, reason="empty_response")

    m = _JSON_RE.search(raw)
    if m is None:
        raw_lower = raw.lower()
        # Известный шаблон отказа
        ...
        # Короткий не-JSON ответ без JSON-структуры → вероятный неизвестный отказ.
        # YandexGPT при нормальной работе всегда возвращает JSON согласно промту;
        # если JSON нет, модель либо отказала, либо вышла из роли.
        # Порог 300 символов: краткие отказы ≤ 300, длинные технические сбои > 300.
        if len(raw) <= 300:
            log.warning("spam_llm.likely_refusal", raw_preview=raw[:120])
            return SpamLLMVerdict(is_spam=True, reason="llm_likely_refused")
